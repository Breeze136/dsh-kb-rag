...
- The package also ships an **explicitly-invoked installer command**, `dsh-kb-rag-install`
  (`install.mjs`, declared in `bin`). It is a 37-line dispatcher only: it forwards to the bundled
  `scripts/install.sh` (macOS/Linux) or `scripts/install.ps1` (Windows, translating bash-style
  flags), and those scripts do the actual work. It never runs on install — `package.json` still
  declares **no lifecycle install scripts** (`preinstall`/`postinstall` etc.); the bin executes
  only when you run it yourself via `npx --yes --package dsh-kb-rag -c "dsh-kb-rag-install ..."`
  (or the scripts directly). The scripts spawn `python` (deps + engine smoke test) and `npm`/`dsh`
  when those steps are enabled. Read them before running, same as any install script.
...
- **Network**: on first use, the embedding/reranker models download from Hugging Face
  ...
  (`scripts/install.ps1` / `scripts/install.sh`) also run `pip`/`npm`/`dsh` — read them before
