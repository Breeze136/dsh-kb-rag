...
## [1.2.0] - 本地元数据增强 + 图注坐标（未发布）

- 元数据本地增强（纯离线）：清理 `Microsoft Word - ` 前缀与 `untitled`/中文占位标题；PDF 元数据不可用时回退首页标题启发式；作者占位符清理；年份合理性校验（1900–今年+1，含 creationDate）
- 图注坐标关联（保守精确匹配）：命中正文段若引用 `Fig. N`（仅同文档、编号完全一致），结果附带"↳ 图注坐标: Fig. N — 图注原文"；匹配不到则不显示，绝不猜测
