...
dsh plugin --profile web add dsh-kb-rag@1.6.4    # 或钉版本
```
...
> 注意：不要在 profile 目录中执行 `npm install dsh-kb-rag`——那会铺出 npm 布局的 `node_modules`，与 pnpm 的符号链接结构/锁文件冲突。`npm install` 只适用于完全手动、不经 `dsh plugin` 管理的部署。
...
1. 首次检索会自动弹出**查询范围**选择（封闭库 / 库+全网 / 仅全网）与**检索深度**选择（快速检索 / 深度检索，默认推荐深度），也可随时 `kb_scope` 切换；"严格只按库内回答"= 严格模式。
2. 提问："石墨烯是怎么用化学气相沉积合成的？" → `kb_rag`（默认深度检索：精排 + 引文关联 + 相关文献，Top-3 证据，自动带编号引用 + 可点击 DOI）。
3. 找片段："搜 graphene CVD on copper" → `kb_search`（默认快速检索：混合召回直出，跳过精排/引文扩展，亚秒级；要完整链路传 `depth="deep"`）。
4. 命中带页码时渲染为「§章节 · p.N」——库里有 Zotero 来源的 PDF，可直接 `zotero://open-pdf?page=N` 一键跳页。
5. 证据下方 `↳ 引文补充`：该证据参考文献里**已在库内**的条目标 `[库内]`（可继续检索/引用），库外的给出补库线索。
...
| 严格模式 | "严格只按库内回答" |
| 检索深度 | "用快速检索" / "用深度检索"（kb_scope 的 depth 参数，会话级生效） |
| 大库入库 | DSH 侧是同步长任务（工具时限 30 分钟）；MCP 侧待处理文件数超过 25 自动转后台作业，用 `kb_status` 轮询 |
